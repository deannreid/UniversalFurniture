...
import universalfurniture.core.DefaultProps;
...
public class Version implements Runnable {
		...
		public static boolean needsUpdateNoticeAndMarkAsSeen() {
			...
			Property property = UniversalFurnitureCore.mainConfiguration.get("vars", "version.seen", VERSION);
			...
			UniversalFurnitureCore.mainConfiguration.save();
			return true;}
			...
			public static void versionCheck() {
				try {
						...
						if (line.startsWith(mcVersion)) {
							if (line.contains(DefaultProps.MOD)) {
								...
								if (line.endsWith(VERSION)) {
									UniversalFurnitureCore.ufLog.finer("Using the latest version [" + getVersion() + "] for Minecraft " + mcVersion);
					...
					UniversalFurnitureCore.ufLog.warning("Using outdated version [" + VERSION + " (build:" + BUILD_NUMBER + ")] for Minecraft " + mcVersion
							+ ". Consider updating.");
					...
					UniversalFurnitureCore.ufLog.warning("Unable to read from remote version authority.");
					UniversalFurnitureCore.ufLog.warning(e.toString());
			...
			public static String[] grabChangelog(String version) {
				...
				try {
					...
					ex.printStackTrace();
					UniversalFurnitureCore.ufLog.warning("Unable to read changelog from remote site.");
				}
			...
			public void run() {
				...
				UniversalFurnitureCore.ufLog.info("Beginning version check");
				...
				try {
					while ((count < 3) && ((currentVersion == null) || (currentVersion == EnumUpdateState.CONNECTION_ERROR))) {
						...
						if (currentVersion == EnumUpdateState.CONNECTION_ERROR) {
							UniversalFurnitureCore.ufLog.info("Version check attempt " + count + " failed, trying again in 10 seconds");
							Thread.sleep(10000);
						}
					}
				...
				if (currentVersion == EnumUpdateState.CONNECTION_ERROR) {
					UniversalFurnitureCore.ufLog.info("Version check failed");
				}
